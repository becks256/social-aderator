...
export async function renderCreative(input: RenderInput): Promise<Buffer> {
          ...
          .composite([
            {
              input: await sharp({
                create: {
                  width: t.hero.width === 1056 ? canvas.width - 1056 : 0,
                  height: canvas.height,
                  channels: 4,
                  background: "#1e293b",
                },
  ...
  const packshot = await sharp(input.packshotBuffer)
    .resize(t.packshot.width, t.packshot.height, {
      fit: "contain",
  ...
  const logo = await sharp(input.logoBuffer)
    .resize(t.logo.width, t.logo.height, {
      fit: "contain",
      background: { r: 0, g: 0, b: 0, alpha: 0 },
    })
    .png()
  ...
  const headlineSvg = Buffer.from(
    buildSvgText(input.copy.headline, t.text, canvas),
  );
  const bodySvg = Buffer.from(
    buildSvgText(input.copy.body, t.bodyText, canvas),
  );
  ...
  if (input.copy.disclaimer) {
    const disclaimerSvg = Buffer.from(
      buildSvgText(input.copy.disclaimer, t.disclaimer, canvas),
    );
