...
export default function RunClient() {
  ...
  async function runPipeline() {
    ...
    const url = aiCompliance
      ? "/api/pipeline/run"
      : "/api/pipeline/run?compliance=0";
      ...
      >
        <div className="flex items-center gap-1.5">
          {status === "done" ? (
            ...
            <svg
              className="animate-spin w-3.5 h-3.5 text-blue-400 flex-shrink-0"
              viewBox="0 0 24 24"
              fill="none"
            >
              <circle
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 px-6">
      ...
      <p className="text-sm text-gray-400 mb-8">
        Paste a brief, upload assets, then run the pipeline.
      </p>
                ...
                {assetTile(
                  ...
                  (f) =>
                    handleUpload(
                      parsed.campaign.id,
                      "campaign-logo",
                      parsed.campaign.logo ?? `${parsed.campaign.id}-logo.png`,
                      f,
                    ),
                )}
              </div>
                  ...
                  {assetTile(
                    ...
                    (f) =>
                      handleUpload(
                        parsed.campaign.id,
                        `${product.id}-packshot`,
                        product.packshot ?? `${product.id}-packshot.png`,
                        f,
                      ),
                  )}
                  {product.logo &&
      ...
      <div className="flex items-center justify-between mb-4">
        <label className="flex items-center gap-2.5 cursor-pointer select-none">
          ...
          >
            <span
              className={`absolute top-0.5 w-3.5 h-3.5 bg-white rounded-full shadow transition-transform ${aiCompliance ? "translate-x-4" : "translate-x-0.5"}`}
            />
          </div>
        ...
        <div className="mt-6 border border-gray-100 rounded-lg overflow-hidden">
            ...
            {running ? (
              <>
                ...
                <span className="text-xs font-medium text-gray-600">
                  Running &mdash; {artifactCount} artifact
                  {artifactCount !== 1 ? "s" : ""} complete
                </span>
              </>
            ) : (
              <>
                ...
                <span className="text-xs font-medium text-gray-600">
                  Done &mdash; {artifactCount} artifact
                  {artifactCount !== 1 ? "s" : ""}
                </span>
              </>
            )}
          ...
          <div className="px-4 py-3 space-y-1.5">
            {log.map((line, i) => {
              const isSubError =
                line.status === "error" &&
                log.slice(0, i).some((e) => e.step === line.step);
              return (
                <div
                  key={i}
                  className={`flex items-center gap-2.5 text-xs ${isSubError ? "pl-6" : ""}`}
                >
                  {isSubError ? (
                    <span className="text-gray-300 flex-shrink-0">↳</span>
                  ) : (
                    <span className="font-mono text-gray-400 w-24 flex-shrink-0">
                      {line.step}
                    </span>
                  )}
                  {line.status === "running" ? (
