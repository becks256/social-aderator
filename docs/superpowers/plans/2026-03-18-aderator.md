...
import { reviewCompliance as geminiReviewCompliance } from '@/lib/gemini'
        ...
        for (const product of brief.products) {
          ...
          const heroBuffer = assets.heroPath
            ? (await readAsset(brief.campaign.id, assets.heroPath.includes('/') ? assets.heroPath.split('/').pop()! : assets.heroPath))
              ?? await (await import('@/lib/gemini')).generatePlaceholderImage(product.name, '#4a90d9', 1080, 1080)
            : await (await import('@/lib/gemini')).generatePlaceholderImage(product.name, '#4a90d9', 1080, 1080)
...
export default function ReviewClient({ initialManifests }: Props) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (res.ok) {
        ...
        setManifests(fresh)
        if (fresh.length > manifests.length) {
  ...
  const handleFilterChange = (key: string, value: string) => {
    ...
    if (key === 'market') setFilterMarket(value)
    if (key === 'ratio') setFilterRatio(value)
    if (key === 'state') setFilterState(value)
  }
